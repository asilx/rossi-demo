...
int main(int argc, char **argv)
{
  ...
  while (!pickup_client.waitForResult(ros::Duration(60.0)))
  {
  ...
  //get the most confident database model
  unsigned int n_potential_models = processing_call.response.graspable_objects.at(0).potential_models.size();
  int most_confident_model_index = -1;
  float max_confidence = 0;
  for(unsigned int i=0; i< n_potential_models; i++)
  {
  	float model_confidence = processing_call.response.graspable_objects.at(0).potential_models[i].confidence; 
  	if( model_confidence >= max_confidence)
  	{
  		max_confidence = model_confidence;
  		most_confident_model_index = i;
  	}
	}
	pickup_location = processing_call.response.graspable_objects.at(0).potential_models[most_confident_model_index].pose;
  if(most_confident_model_index == -1)
  {
