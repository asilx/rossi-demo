...
void BehaviorModule::home() {
	...
	action->pushAction(home_x, home_orient, "open_hand");
	action->checkActionsDone(f, true);
...
void BehaviorModule::tuckArms() {
	...
	if (Network::isConnected(
			...
			"/icub/cartesianController/left_arm/command:i"))
		left_arm_cart_solver_active = true;

	if (Network::isConnected(
			"/actionPrimitivesMod/right_arm/position/command:o",
			"/icub/cartesianController/right_arm/command:i"))
	...
	if (!sim) {
		if (left_arm_cart_solver_active) {
			...
			Network::disconnect("/actionPrimitivesMod/left_arm/position/rpc:o",
					"/icub/left_arm/rpc:i");
			Network::disconnect("/icub/left_arm/state:o",
		...
		if (right_arm_cart_solver_active) {
			Network::disconnect(
					...
					"/icub/cartesianController/right_arm/command:i");
			Network::disconnect(
					"/actionPrimitivesMod/right_arm/position/rpc:o",
					"/icub/right_arm/rpc:i");
			Network::disconnect("/icub/right_arm/state:o",
	...
	if (!sim) {
		if (left_arm_cart_solver_active) {
			...
			Network::connect("/actionPrimitivesMod/left_arm/position/rpc:o",
					"/icub/left_arm/rpc:i");
			Network::connect("/icub/left_arm/state:o",
					"/actionPrimitivesMod/left_arm/position/state:i");
		}
		...
		if (right_arm_cart_solver_active) {
			...
			Network::connect("/actionPrimitivesMod/right_arm/position/rpc:o",
					"/icub/right_arm/rpc:i");
			Network::connect("/icub/right_arm/state:o",
					"/actionPrimitivesMod/right_arm/position/state:i");
		}
	} else {
		//TODO for simulator
