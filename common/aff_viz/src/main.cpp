...
updateAffordanceMarkers (aff_msgs::ObjectOfInterest& ooi_, visualization_msgs::MarkerArray& aff_markers)
{
  ...
  start.x = ooi_.object_center[0];
  start.y = ooi_.object_center[1];
  start.z = ooi_.object_center[2];

  geometry_msgs::Point end;
  end.x = ooi_.object_center[0];
  end.y = 0.50;
  end.z = 0.30;
  ...
  for (uint8_t i = 0; i < ooi_.affordances.size (); i++)
  {
    ...
    marker.pose.position.x = aff_markers.markers[0].points[1].x;
    marker.pose.position.y = aff_markers.markers[0].points[1].y;
    marker.pose.position.z = aff_markers.markers[0].points[1].z + 0.07 * (i);
...
//    marker.pose.position.x -= (ooi_.object_size[0]);
    ...
    aff_markers.markers[i + 1] = marker;
  }
}
