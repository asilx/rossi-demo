...
//  ROS_INFO("finished -tuck_arms- action");
  ...
  while (n->ok ())
  {
    ...
    {
      for(uint8_t i=0;i<ooi_affordances.affordances.size();i++)
        std::cout<<ooi_affordances.affordances[i]<<std::endl;
      std::cout<<ooi_affordances.object_center[0]<<" "
