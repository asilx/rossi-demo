...
const float object_center[]={-0.3, -0.2, 0.1};
                      ...
                      visualization_msgs::MarkerArray bb_texts)
{
  ...
  for (uint i = 0; i < indexed_clusters.clusters.size (); i++)
  {
    ...
    marker.id = i;
    std::stringstream s;
//    s << indexed_clusters.cluster_indices[i];
    s << i;
    marker.text = s.str ();
...
filterOutNonTableClusters (std::vector<sensor_msgs::PointCloud>& clusters, const tabletop_object_detector::Table& table)
{
  for (uint i = 0; i < clusters.size (); i++)
  {
    ...
    if (min_x < table.x_min || max_x > table.x_max || min_y < table.y_min || max_y > table.y_max || min_z
        > table.pose.pose.position.z + 0.05)
    {
      clusters.erase (clusters.begin () + i);
      i--;
    }
...
int getClusterByLabel(int8_t inputIndex)
{
	for(int i = 0; i < indexed_clusters.cluster_indices.size(); i++)
	{
		if(indexed_clusters.cluster_indices[i] == (int)inputIndex)
		{
			return i;
		}
	}
	...
	std::cout << "getClusterByLabel: could not find the proper index! check the implementation" << std::endl;
...
//      }
  ...
  else if (request.task == feature_manager::Perception::Request::EXTRACT_EFFECT)
  {
    ...
    if(featurize->isFinal())
    {
        ...
        // TODO: get cluster by label

      	int i = getClusterByLabel(objectOfInterest);

        //for (uint8_t i = objectOfInterest; i < objectOfInterest; i++)
        {
          sensor_msgs::PointCloud2 pc2_cluster;
          sensor_msgs::convertPointCloudToPointCloud2 (segmentation_call.response.clusters[i], pc2_cluster);
          featurize->extractAndGetFeatures (pc2_cluster);
          ...
          std::cout << "A new init vector: " << endl;
          for (uint8_t j = 0; j < featureCount; j++)
          {
            std::cout << dummy[j] << " ";
          }

          featurize->featureLogger->logSingleData (dummy, i); // store the initial vector just in case!
          dummy = featurize->getEffectVector ();
          ...
          featurize->effectLogger->logSingleData (dummy, i,request.arg); // hence effect for the ith object with label request.arg!
          featurize->setInitVector (); // just for future use, save the final features as the next initial features
          featurize->setFinal (false);
          objectOfInterest = -1;
    ...
    {
      std::cout << "It seems the initial vector was not set at this instance. Would you please check your implementation & execution ?" << std::endl;
    }
...
init ()
{
  ...
  cluster_outrem.setRadiusSearch (0.02);
  cluster_outrem.setMinNeighborsInRadius (30);

  pass_through_x.setFilterFieldName ("x");
  pass_through_x.setFilterLimits (-0.90, -0.1);
  pass_through_y.setFilterFieldName ("y");
  pass_through_y.setFilterLimits (-0.6, 0.6);
