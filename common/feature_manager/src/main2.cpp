...
ros::Publisher pub_arm_detection;
...
void
publishArmDetectionResults (const std::vector<sensor_msgs::PointCloud> &clusters)//tabletop_object_detector::TabletopSegmentationResponse& response)
{
  tabletop_object_detector::TabletopDetectionResult msg;
  msg.clusters = clusters;

  pub_arm_detection.publish (msg);
}
...
filterOutNonTableClusters (std::vector<sensor_msgs::PointCloud>& clusters, const tabletop_object_detector::Table& table)
{
  ...
  std::vector<sensor_msgs::PointCloud> arm_cluster;
  double max_y_size = 0;
  uint max_y_ind = -1;
  uint count = 0;
  for (uint i = 0; i < clusters.size (); i++)
  {
    ...
    {
      arm_cluster.push_back(clusters[i]);
      clusters.erase(clusters.begin () + i);
      count++;
      i--;
    }
  ...
  for (uint i = 0; i < arm_cluster.size (); i++)
  {
    ...
    for (uint j = 0; j < arm_cluster[i].points.size (); j++)
    {
      if (isnan (arm_cluster[i].points[j].x) || isnan (arm_cluster[i].points[j].y) || isnan (arm_cluster[i].points[j].z))
        continue;

      if (arm_cluster[i].points[j].x < min_x)
        min_x = arm_cluster[i].points[j].x;

      if (arm_cluster[i].points[j].x > max_x)
        max_x = arm_cluster[i].points[j].x;

      if (arm_cluster[i].points[j].y < min_y)
        min_y = arm_cluster[i].points[j].y;
      if (arm_cluster[i].points[j].y > max_y)
        max_y = arm_cluster[i].points[j].y;

      if (arm_cluster[i].points[j].z < min_z)
        min_z = arm_cluster[i].points[j].z;
      if (arm_cluster[i].points[j].z > max_z)
        max_z = arm_cluster[i].points[j].z;
    }
    size_y = max_y - min_y;
  ...
  for (uint i = 0; i < arm_cluster.size (); i++)
  {
    if (i != max_y_ind)
    {
      clusters.erase (arm_cluster.begin () + i);
      i--;
    }
  }
}
...
percept ()
{
  if (module_activation)//module is activated
  {
    if (pointcloud_rcvd)//
    {
      ...
      std::vector<sensor_msgs::PointCloud> armCluster = filterOutNonTableClusters (segmentation_call.response.clusters, segmentation_call.response.table);
...
init ()
{
  ...
  pub_arm_detection
      = nh->advertise<tabletop_object_detector::TabletopDetectionResult> ("/arm_result", 10);
