...
namespace pcl{
	...
	struct Feature{
		...
		Feature& operator =(const Feature &tmp){
			min = tmp.min;
			max = tmp.max;
			mean = tmp.mean;
			var = tmp.var;
		...
		void 	print(){
			std::cout<<"min :"<<setiosflags(ios::fixed) << setprecision(3) << setw(7)<<min<<"\t"
					 <<"max :"<<setiosflags(ios::fixed) << setprecision(3) << setw(7)<<max<<"\t"
					 <<"mean:"<<setiosflags(ios::fixed) << setprecision(3) << setw(7)<<mean<<"\t"
					 <<"var :"<<setiosflags(ios::fixed) << setprecision(3) << setw(7)<<var<<std::endl;
			std::cout<<"hist:";
			for(int i=0;i<histogram_n_bins;i++)
		...
		Feature calculate(std::vector<float> const& data, bool apply_itself, float min_possible_val=MIN_FEATURE_VALUE, float max_possible_val=MAX_FEATURE_VALUE){
			...
			for(unsigned int i=0;i<histogram_n_bins;i++) tmp_histogram[i]=0;
			if(histogram_n_bins)
			{
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
		pcl::Feature<RANGE_FEATURES_HISTOGRAM>,
		(float, min,	min)
		(float, max,	max)
		(float, mean,	mean)
		(float,	var,	var)
...
POINT_CLOUD_REGISTER_POINT_STRUCT(
		pcl::Feature<SURFACE_FEATURES_HISTOGRAM>,
		(float, min,	min)
		(float, max,	max)
		(float, mean,	mean)
		(float,	var,	var)
