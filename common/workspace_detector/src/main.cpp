...
ros::Publisher pub_workspace_detection;
...
void
publishWorkspaceDetectionResults (tabletop_object_detector::TabletopSegmentationResponseConstPtr response)
{
  aff_msgs::WorkspaceDetection workspace_det_msg;
  workspace_det_msg.table = response->table;
  workspace_det_msg.tabletop.clusters = response->clusters;
  workspace_det_msg.tabletop.result = response->result;
}
...
main (int argc, char** argv)
{
  ...
  pub_workspace_detection = nh->advertise<aff_msgs::WorkspaceDetection> ("workspace_detection", 10);
  ...
  while (nh->ok ())
  {
    ...
    if (module_activation)
    {
      ...
      publishWorkspaceDetectionResults (
          (tabletop_object_detector::TabletopSegmentationResponseConstPtr)
          &segmentation_call.response);
